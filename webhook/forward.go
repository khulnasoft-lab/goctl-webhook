...
func NewCmdForward() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ goctl webhook forward --events=issues --repo=monalisa/smile --url="http://localhost:9999/webhooks"
			$ goctl webhook forward --events=issues --org=github --url="http://localhost:9999/webhooks"
		`),
		RunE: func(c *cobra.Command, _ []string) error {
			...
			if envHost := os.Getenv("GOCTL_HOST"); envHost != "" && !c.Flags().Changed("github-host") {
				githubHost = envHost
			}

			authToken, err := authTokenForHost(githubHost)
			if err != nil {
				return fmt.Errorf("fatal: error fetching goctl token: %w", err)
			}
